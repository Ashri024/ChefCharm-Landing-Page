...
ChefCharmIntro.propTypes = {
  setIntroTl: PropTypes.func
}

function ChefCharmIntro({setIntroTl}) {
...
useEffect(() => {
  ...
  setIntroTl(tl)
    ...
    }).to('#welcome', { opacity: 1 })
      .to('#chefSvg', { opacity: 1, left: 0 }, "<") // "<" means start at the same time as the previous animation
      .to('#charmSvg', { opacity: 1, left: 0 }, "<")
      .to('#masterChef', { opacity: 1, translateY: 0 }, "-=1")
  ...
  return (
    <div className="relative z-10 text-white h-screen p-4 overflow-hidden max-h-[740px] max-w-[1600px] mx-auto" id="chefCharmIntro">
