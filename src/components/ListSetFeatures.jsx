  ...
  return (
    ...
    <div className=' absolute opacity-20 top-0 left-0 w-full h-full'></div>
      <div id={mockupParentId} className="laptopMockup relative h-[60%] max-[768px]:h-auto flex items-center justify-center">
        <div id={mockupId} className='laptopMockup h-[550px] max-[1024px]:h-[450px] featureMockups opacity-0 min-[768px]:-translate-x-full'>
          <img src={LaptopMockup} alt="Chef Charm" className="w-full z-20 object-center relative" />
        </div>
      </div>

      <div className='w-full min-[768px]:w-2/3 max-w-[540px] flex flex-col justify-around items-center h-[60%]'>
        <div style={{ padding:"1rem", position:"relative",height:"50%"}} className=' flex flex-col justify-center'>
          <div className='relative min-w-[465px] max-[1024px]:min-w-[310px] max-[768px]:top-0'>
            ...
            <div className="relative whatIs min-[768px]:translate-x-1/2 opacity-0 featureHeading" id={headingId}>
              {heading}
            </div>
          </div>
            ...
            <ul id={listId} className="relative max-w-[90%] featuresAboutList max-[768px]:max-w-full">
             {isTech? 
                listArray.map((listItem,index) => {
                  return (
                     <li key={index} className='mb-4 min-[768px]:translate-x-1/2 opacity-0'><strong>{listItem.title}:</strong> {listItem.text}</li>
                  )
                })
             :
                listArray.map((listItem,index) => {
                  return (
                     <li key={index} className='mb-4 min-[768px]:translate-x-1/2 opacity-0'>{listItem}</li>
                  )
                })
             }
            </ul>
